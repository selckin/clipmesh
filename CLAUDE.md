...
- **`sync.rs`** — `SyncEngine`, the brain. Its `run` loop selects over the local clipboard watch, inbound mesh messages, peer-connect events, and rules-changed pings. Handles echo suppression (compare incoming vs. current hash per selection), debounce, `Direction` (send/receive/both), payload caps, and content filtering. **Ordering everywhere — live updates and reconnect resync alike — is the hybrid logical clock `(stamp, origin)`** (`ContentState`); higher stamp wins, `origin` (node ID) breaks ties. Password-manager contents (`x-kde-passwordManagerHint = secret`) are dropped before broadcast (and, while `exclude_sensitive` is on, never re-owned). Two optional capture-side features: `synthesize_text_plain` back-fills `text/plain` (+`;charset=utf-8`) from a legacy `UTF8_STRING`/`STRING`/`TEXT` atom when none exists, and `take_ownership` re-offers each watched selection so clipmesh owns it (surviving the source app) — and, with synthesis, makes the back-filled `text/plain` paste on the origin host too. A separate **local selection bridge** (`link_selections`, `bridge_from`) optionally mirrors the local CLIPBOARD↔PRIMARY selections on one host — distinct from `sync_primary` (which syncs each selection across the mesh). It bridges only *local* user changes: it reconciles against the partner's actual content (so an out-of-band drift is re-mirrored, and the mirror's own echo terminates), and content the engine itself wrote (an inbound mesh apply, content restored at startup, or an ownership rewrite) is recorded in `self_written` and skipped one-shot in `process_local_change`, so engine-written content is never re-broadcast, re-bridged, or re-owned. A *local* mirror write's own watch event still rides the normal broadcast path, so a synced partner is fed to the mesh like any other local change.
