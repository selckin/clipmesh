...
- **`sync.rs`** — `SyncEngine`, the brain.
  ...
  - **Echo suppression of the engine's own writes:** content the engine itself wrote — an inbound mesh apply, content restored at startup, an ownership rewrite, or a bridge mirror — is recorded in `last_written` and its watch echo is dropped one-shot during `handle_batch`'s classification, so engine-written content is never re-broadcast, re-bridged, or re-owned (this is what makes propagation terminate). `write_selection` is the **single** path from the engine to `Clipboard::write_offer` — it records the marker before writing and rolls it back if the write fails, so a new engine-write path cannot forget the record and re-drive propagation. Add engine writes there, not by calling `write_offer` directly. A mesh-synced mirror target is broadcast **directly** by `plan_batch` (not via its echo), so a synced partner is still fed to the mesh like any other local change. See `docs/superpowers/specs/2026-06-20-bridge-write-consolidation-design.md`.
